...
Şekil~\ref{fig:nokta-tahmin-akisi}, araştırma sorusundan raporlanabilir bir
tahmine giderken verilmesi gereken kararları birbirine bağlar. Akışın ilk üç
adımı belirsiz bırakılırsa doğru hesaplanan bir tahmin bile yanlış hedefi
yanıtlayabilir.
...
\begin{tikzpicture}[
  font=\small,
  box/.style={draw=PrismBlue!75!black, rounded corners=2pt,
    fill=PrismLight, align=center, minimum height=9mm, text width=3.05cm},
  arrow/.style={-{Stealth[length=2mm]}, thick, draw=PrismBlue!80!black},
  node distance=7mm and 8mm
]
\node[box] (evren) {Hedef evreni, zamanı ve ölçümü tanımla};
\node[box, right=of evren] (nicelik) {Hedef niceliği seç\\ortalama, oran, medyan, fark};
\node[box, right=of nicelik] (tasarim) {Örnekleme, eksik veri ve ağırlıkları belirle};
\node[box, below=of tasarim] (tahmin) {Uygun tahmin ediciyi ve hesaplama kuralını seç};
\node[box, left=of tahmin] (rapor) {Tahmin $+$ standart hata / güven aralığı $+$ kapsam};
\draw[arrow] (evren) -- (nicelik);
\draw[arrow] (nicelik) -- (tasarim);
\draw[arrow] (tasarim) -- (tahmin);
\draw[arrow] (tahmin) -- (rapor);
\end{tikzpicture}
\caption{Araştırma hedefinden belirsizliği belirtilmiş tahmine giden akış.}
\label{fig:nokta-tahmin-akisi}
\end{figure}
...
Şekil~\ref{fig:tahmin-edici-secimi}, önce bilimsel hedefi koruyup ardından
tahmin edicinin örnekleme davranışını denetleyen kısa bir seçim haritasıdır.

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[
  font=\small,
  root/.style={draw=PrismBlue!80!black, rounded corners=2pt,
    fill=PrismBlue!12, align=center, minimum height=9mm, text width=3.2cm},
  choice/.style={draw=PrismBlue!65!black, rounded corners=2pt,
    fill=PrismLight, align=center, minimum height=10mm, text width=2.05cm,
    font=\footnotesize},
  check/.style={draw=PrismBlue!80!black, rounded corners=2pt,
    fill=PrismResearchGreen, align=center, minimum height=9mm, text width=7.4cm},
  arrow/.style={-{Stealth[length=2mm]}, thick, draw=PrismBlue!75!black},
  node distance=8mm
]
\node[root] (hedef) {Hangi evren özelliği hedefleniyor?};
\node[choice] at (-4,-1.8) (mean) {Ortalama düzey\\$\bar x$};
\node[choice] at (-1.33,-1.8) (prop) {Olay oranı\\$\hat p=x/n$};
\node[choice] at (1.33,-1.8) (median) {Tipik sıra / sağlam merkez\\medyan};
\node[choice] at (4,-1.8) (diff) {Gruplar veya zamanlar arası fark};
\node[check] at (0,-3.7) (kontrol) {Her aday için: yanlılık, standart hata, MSE, tutarlılık ve aykırı değer duyarlılığını değerlendir};
\draw[arrow] (hedef) -- (mean);
\draw[arrow] (hedef) -- (prop);
\draw[arrow] (hedef) -- (median);
\draw[arrow] (hedef) -- (diff);
\draw[arrow] (mean) -- (kontrol);
\draw[arrow] (prop) -- (kontrol);
\draw[arrow] (median) -- (kontrol);
\draw[arrow] (diff) -- (kontrol);
\end{tikzpicture}
\caption{Hedef nicelikten tahmin ediciye ve nitelik denetimine geçiş.}
\label{fig:tahmin-edici-secimi}
\end{figure}
...
\input{chapters/r/b07}
\input{chapter-exercises/05-nokta-tahmini}
...
\bolumbaglantisi{chapters/pdr/07-nokta-tahmini.tex}