...
Araştırma hedefi sıra veya yön karşılaştırmasını gerektiriyorsa tasarım yapısı
uygun test ailesini belirler. Şekil~\ref{fig:nonparametrik-secim}, yaygın beş
durumu tek haritada toplar; ortalama farkını hedefleyen sorularda ise önce
Welch, sağlam standart hata, permütasyon veya bootstrap seçenekleri
...
\begin{tikzpicture}[
  ...
  root/.style={draw=PrismBlue!80!black, rounded corners=2pt,
    fill=PrismBlue!12, align=center, minimum height=9mm, text width=3.5cm},
  design/.style={draw=PrismBlue!65!black, rounded corners=2pt,
    fill=PrismLight, align=center, minimum height=9mm, text width=2.65cm},
  method/.style={draw=PrismBlue!75!black, rounded corners=2pt,
    fill=PrismResearchGreen, align=center, minimum height=9mm, text width=2.65cm},
  arrow/.style={-{Stealth[length=2mm]}, thick, draw=PrismBlue!80!black},
  node distance=7mm
]
\node[root] (question) {Sıra, yön veya tekdüze ilişki hedefi nedir?};
\node[design] at (-2.6,-1.5) (one) {Tek örneklem veya eşleştirilmiş fark};
\node[method] at (2.6,-1.5) (signed) {İşaret testi veya Wilcoxon işaretli sıra};
\node[design] at (-2.6,-2.8) (two) {İki bağımsız grup};
\node[method] at (2.6,-2.8) (mw) {Mann--Whitney};
\node[design] at (-2.6,-4.1) (many) {Üç veya daha fazla bağımsız grup};
\node[method] at (2.6,-4.1) (kw) {Kruskal--Wallis\\ardından Dunn--Holm};
\node[design] at (-2.6,-5.4) (relation) {İki değişken arasında tekdüze ilişki};
\node[method] at (2.6,-5.4) (spear) {Spearman korelasyonu};
\coordinate (spine) at (0,-0.8);
\draw[arrow] (question) -- (spine);
\draw[thick, draw=PrismBlue!80!black] (spine) -- (0,-5.4);
\draw[arrow] (0,-1.5) -- (one);
\draw[arrow] (0,-2.8) -- (two);
\draw[arrow] (0,-4.1) -- (many);
\draw[arrow] (0,-5.4) -- (relation);
\draw[arrow] (one) -- (signed);
\draw[arrow] (two) -- (mw);
\draw[arrow] (many) -- (kw);
\draw[arrow] (relation) -- (spear);
\end{tikzpicture}
\caption{Araştırma yapısına göre parametrik olmayan yöntem ailesi seçimi.}
\label{fig:nonparametrik-secim}
\end{figure}
...
\input{chapters/r/nonparametric}
\input{chapter-exercises/13-nonparametrik}
...
\bolumbaglantisi{chapters/pdr/13-nonparametrik.tex}